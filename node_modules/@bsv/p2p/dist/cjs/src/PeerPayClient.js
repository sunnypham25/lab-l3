...
function safeParse(input) {
    try {
        ...
        logger_js_1.Logger.error('[PP CLIENT] Failed to parse input in safeParse:', input);
...
class PeerPayClient extends MessageBoxClient_js_1.MessageBoxClient {
    ...
    get authFetchInstance() {
        if (this._authFetchInstance === null || this._authFetchInstance === undefined) {
            this._authFetchInstance = new sdk_1.AuthFetch(this.peerPayWalletClient);
        }
    ...
    async createPaymentToken(payment) {
        ...
        logger_js_1.Logger.log(`[PP CLIENT] Derivation Prefix: ${derivationPrefix}`);
        logger_js_1.Logger.log(`[PP CLIENT] Derivation Suffix: ${derivationSuffix}`);
        ...
        logger_js_1.Logger.log(`[PP CLIENT] Derived Public Key: ${derivedKeyResult}`);
        ...
        logger_js_1.Logger.log(`[PP CLIENT] Locking Script: ${lockingScript}`);
        ...
        logger_js_1.Logger.log('[PP CLIENT] Payment Action:', paymentAction);
        return {
            customInstructions: {
                derivationPrefix,
                derivationSuffix
    ...
    async sendLivePayment(payment) {
        ...
        try {
            ...
            logger_js_1.Logger.warn('[PP CLIENT] sendLiveMessage failed, falling back to HTTP:', err);
    ...
    async listenForLivePayments({ onPayment }) {
        await this.listenForLiveMessages({
            ...
            onMessage: (message) => {
                logger_js_1.Logger.log('[MB CLIENT] Received Live Payment:', message);
                const incomingPayment = {
                    messageId: message.messageId,
                    sender: message.sender,
                    token: safeParse(message.body)
                };
                logger_js_1.Logger.log('[PP CLIENT] Converted PeerMessage to IncomingPayment:', incomingPayment);
    ...
    async acceptPayment(payment) {
        try {
            logger_js_1.Logger.log(`[PP CLIENT] Processing payment: ${JSON.stringify(payment, null, 2)}`);
            ...
            logger_js_1.Logger.log(`[PP CLIENT] Payment internalized successfully: ${JSON.stringify(paymentResult, null, 2)}`);
            logger_js_1.Logger.log(`[PP CLIENT] Acknowledging payment with messageId: ${payment.messageId}`);
            await this.acknowledgeMessage({ messageIds: [payment.messageId] });
            return { payment, paymentResult };
        }
        catch (error) {
            logger_js_1.Logger.error(`[PP CLIENT] Error accepting payment: ${String(error)}`);
    ...
    async rejectPayment(payment) {
        logger_js_1.Logger.log(`[PP CLIENT] Rejecting payment: ${JSON.stringify(payment, null, 2)}`);
        if (payment.token.amount - 1000 < 1000) {
            logger_js_1.Logger.log('[PP CLIENT] Payment amount too small after fee, just acknowledging.');
            try {
                logger_js_1.Logger.log(`[PP CLIENT] Attempting to acknowledge message ${payment.messageId}...`);
                if (this.authFetch === null || this.authFetch === undefined) {
                    logger_js_1.Logger.warn('[PP CLIENT] Warning: authFetch is undefined! Ensure PeerPayClient is initialized correctly.');
                }
                logger_js_1.Logger.log('[PP CLIENT] authFetch instance:', this.authFetch);
                const response = await this.acknowledgeMessage({ messageIds: [payment.messageId] });
                logger_js_1.Logger.log(`[PP CLIENT] Acknowledgment response: ${response}`);
            }
            catch (error) {
                if (error != null &&
                    typeof error === 'object' &&
                    ...
                    logger_js_1.Logger.warn(`[PP CLIENT] Authentication issue while acknowledging: ${error.message}`);
                }
                else {
                    logger_js_1.Logger.error(`[PP CLIENT] Error acknowledging message: ${error.message}`);
                    throw error; // Only throw if it's another type of error
                }
            }
            return;
        }
        logger_js_1.Logger.log('[PP CLIENT] Accepting payment before refunding...');
        await this.acceptPayment(payment);
        logger_js_1.Logger.log(`[PP CLIENT] Sending refund of ${payment.token.amount - 1000} to ${payment.sender}...`);
        await this.sendPayment({
            recipient: payment.sender,
            amount: payment.token.amount - 1000 // Deduct fee
        });
        logger_js_1.Logger.log('[PP CLIENT] Payment successfully rejected and refunded.');
        try {
            logger_js_1.Logger.log(`[PP CLIENT] Acknowledging message ${payment.messageId} after refunding...`);
            await this.acknowledgeMessage({ messageIds: [payment.messageId] });
            logger_js_1.Logger.log('[PP CLIENT] Acknowledgment after refund successful.');
        }
        catch (error) {
            logger_js_1.Logger.error(`[PP CLIENT] Error acknowledging message after refund: ${error.message}`);
        }
    }
    /**
    ...
    async listIncomingPayments() {
        const messages = await this.listMessages({ messageBox: exports.STANDARD_PAYMENT_MESSAGEBOX });
